...
public class GlobalEconomyMain {
	...
	public Country createCountry() {
		...
		System.out.println("Please enter country's president");
		String president = input.nextLine();

		Country newCountry = new Country(name, gdp, population, capital, president);
		return newCountry;
	}
	...
	/*
	 * This method only reads a String that here, will be the name of a country that
	 * you want to use (for printing, injecting money, paying debt, etc.)
	 */
	...
	public void payDebt() {
		...
		Country foundCountry = retrieveCountry(countryName);
		if (foundCountry == null) {
			System.err.println("Error: " + countryName + " is not registered.");
		} else {
			System.out.println("Enter the amount you want to pay");
			double amountDebt = input.nextDouble();
			input.nextLine();
			foundCountry.payDebt(amountDebt);
			if (amountDebt > 0 && amountDebt < foundCountry.getgdp()) {
				System.out.println(countryName + " new gdp is " + foundCountry.getgdp());
			}
		}
	}
	...
	public void editInfo() {
		...
		if (foundCountry != null) {
			...
			System.out.println("3. Change a country's capital.");
			System.out.println("3. Change a capital's mayor.");
			...
			if (optionNum == 1) {
				...
				String newCapital = input.nextLine();
				foundCountry.setCapital(newCapital);
				System.out.println("The country's new capital is: " + newCapital);

			} else if (optionNum == 4) {
				System.out.println("Enter the country's mayor. ");
				String newMayor = input.nextLine();
				foundCountry.setMayor(newMayor);
				System.out.println("The country's new mayor is: " + newMayor);
